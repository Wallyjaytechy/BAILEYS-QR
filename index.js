...
setTimeout(() => {
  ...
  async function WALLYJAYTECH_BAILEYS_QR() {
...
let cc = `╔════◇
║『 *THANKS FOR CHOOSING WALLYJAYTECH-MD* 』
║ _You complete first step to making Bot._
╚════════════════════════╝
╔═════◇
║  『••• 𝗩𝗶𝘀𝗶𝘁 𝗙𝗼𝗿 𝗛𝗲𝗹𝗽 •••』
║ *1.Github:* _github.com/wallyjaytechy_
...
`;
       ...
       if (connection === "close" && lastDisconnect && lastDisconnect.error && lastDisconnect.error.output.statusCode != 401) { WALLYJAYTECH_BAILEYS_QR(); } // IF ANY ERROR< THEN PRINT QR AGAIN
      });
    } catch (err) {console.log(err);await require('child_process').exec('rm -rf auth_info_baileys');process.exit(1);}
  }
  WALLYJAYTECH_BAILEYS_QR();
}, 3000)
